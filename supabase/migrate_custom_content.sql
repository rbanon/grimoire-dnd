...
create table if not exists custom_subclasses (
  ...
  is_public     boolean not null default false,
  author_name   text,
  data          jsonb not null,     -- full CustomSubclass definition
  created_at    timestamptz default now() not null,
  updated_at    timestamptz default now() not null
);
...
drop policy if exists "custom_subclasses: public read"  on custom_subclasses;
drop policy if exists "custom_subclasses: owner insert" on custom_subclasses;
drop policy if exists "custom_subclasses: owner update" on custom_subclasses;
drop policy if exists "custom_subclasses: owner delete" on custom_subclasses;
...
drop trigger if exists set_updated_at on custom_subclasses;
